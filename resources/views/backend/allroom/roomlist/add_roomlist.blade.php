...
<div class="page-content">
    ...
    <form method="POST" action="{{ route('store.roomlist') }}" class="row g-3">
        ...
        <div class="col-md-4">
            ...
            <select name="room_id" id="room_id" class="form-select" required>
                ...
                @foreach ($roomtype as $item) 
                    @if($item->room)
                        <option value="{{ $item->room->id }}" {{ old('room_id') == $item->room->id ? 'selected' : '' }} >{{ $item->name }}</option>
                    @endif
                @endforeach
            </select>
            @error('room_id')
        ...
        <div class="col-md-4">
            <label for="input2" class="form-label">Checkin <span class="text-danger">*</span></label>
            <input type="date" name="check_in" class="form-control" id="check_in" value="{{ old('check_in') }}" required min="{{ date('Y-m-d') }}">
            @error('check_in')
                <span class="text-danger">{{ $message }}</span>
            @enderror
        </div>
        ...
        <div class="col-md-4">
            <label for="input2" class="form-label">CheckOut <span class="text-danger">*</span></label>
            <input type="date" name="check_out" class="form-control" id="check_out" value="{{ old('check_out') }}" required>
            @error('check_out')
                <span class="text-danger">{{ $message }}</span>
            @enderror
        </div>
        ...
        <div class="col-md-4">
            <label for="input3" class="form-label">Room <span class="text-danger">*</span></label>
            <input type="number" name="number_of_rooms" class="form-control" value="{{ old('number_of_rooms') }}" min="1" required>
        ...
        <div class="col-md-4">
            <label for="input4" class="form-label">Guest <span class="text-danger">*</span></label>
     ...
     <h3 class="mt-3 mb-5 text-center">Customer Information </h3>
        ...
        <div class="col-md-4">
            ...
            <input type="text" name="name" class="form-control" id="input5" value="{{ old('name') }}" required>
            @error('name')
                <span class="text-danger">{{ $message }}</span>
            @enderror
        </div>
        ...
        <div class="col-md-4">
            <label for="input5" class="form-label">Phone <span class="text-danger">*</span></label>
